...
\]
(exercise left to the reader; note to self to not leave this as an exercise to reader). 
...
One notices that \(R_{\alpha,0}-\mathrm{id}_4\) is the generator of ASEP at two lattice sites
(note to self that this is not quite correct). It is then
...
\begin{tikzpicture}
   ...
   \node (right) at (5,-5) {\(A \otimes A\)};
   \node (bottom) at (0,-10) {\(A\)};
   \draw[black,-latex,line width=2pt] (top)--(left)  node[midway,above,sloped] () {$\iota \otimes \mathrm{id}$};
   \draw[black,-latex,line width=2pt] (top)--(right) node[midway,above,sloped] () {$\mathrm{id} \otimes \iota$};
   \draw[black,-latex,line width=2pt] (left)--(bottom) node[midway,above] () {$m$};
   \draw[black,-latex,line width=2pt] (right)--(bottom) node[midway,above] () {$m$};
...
A Hopf algebra is a bi--algebra with an involutive anti--homomorphism $S$ such that this diagram
commutes:
